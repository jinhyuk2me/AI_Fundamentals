...
가중치 정규화는 **모델의 가중치가 지나치게 커지지 않도록 제어**하여 과적합을 방지하는 기법입니다.

#### 왜 필요한가?

모델이 훈련 데이터를 너무 잘 외워버리면 (과적합), 새로운 데이터에서는 성능이 급격히 떨어집니다.

**과적합의 주요 원인**:
- 가중치가 너무 커져서 입력의 작은 변화에도 출력이 크게 요동침
- 복잡한 함수를 학습하여 노이즈까지 학습함

**해결책**: 손실 함수에 가중치 크기에 대한 **패널티(penalty)** 를 추가

---

#### 수식으로 이해하기

기본 손실 함수에 정규화 항을 추가합니다:

$$
L_{\text{total}} = L_{\text{data}} + \lambda R(W)
$$

- $L_{\text{data}} = \frac{1}{N} \sum_{i=1}^{N} \ell(y_i, \hat{y}_i)$: 원래의 손실 (Cross Entropy, MSE 등)
- $R(W)$: 정규화 항 (Regularization Term)
- $\lambda$: 정규화 강도 (Regularization Strength) — 하이퍼파라미터
- $W$: 모델의 모든 가중치

**의미**: "학습이 잘 되면서도 가중치가 너무 커지지 않게 균형을 맞추자"
...
$$
R(W) = \frac{1}{2} \|W\|_2^2 = \frac{1}{2} \sum_{j} W_j^2
$$
...
$$
W \leftarrow W - \eta \left( \nabla_W L_{\text{data}} + \lambda W \right)
$$

$\lambda W$ 항이 **가중치 축소 효과(weight decay)**로 작용합니다.
...
$$
L_{\text{total}} = L_{\text{data}} + \lambda \sum_{j} |W_j|
$$

**특징**:
- 불필요한 가중치를 **정확히 0**으로 만듦
- **희소성(Sparsity)** 유도 → 특징 선택(Feature Selection) 효과
- L2보다 공격적으로 가중치 제거

**언제 사용하나?**:
- 특징이 많고 중요한 특징만 선택하고 싶을 때
- 모델 해석이 중요할 때 (어떤 특징이 중요한지 명확히)

---

#### Elastic Net

L1과 L2를 결합하여 둘의 장점을 취합니다:

$$
L_{\text{total}} = L_{\text{data}} + \lambda_1 \sum_{j} |W_j| + \lambda_2 \sum_{j} W_j^2
$$
...
정규화를 추가하면 Gradient Descent 과정에서:
- 가중치가 커질수록 패널티가 커짐
- 업데이트 시 더 강하게 줄어듦
...
for W in model.parameters():
    W -= lr * (grad_W + lambda_ * W)  # lambda_ * W가 weight decay
```
...
| 효과 | 설명 |
|------|------|
| ✅ 과적합 방지 | 큰 가중치로 인한 과도한 피팅 방지 |
| ✅ 일반화 향상 | 테스트 데이터에서도 안정된 예측 |
| ✅ 수치적 안정성 | Gradient 폭주/소실 완화 |
| ⚠️ 주의사항 | $\lambda$ 너무 크면 과소적합 발생 |
...
W = [0.5, 0.3, 0.2]   W = [0.6, 0.4, 0.0]
  (모두 작아짐)         (약한 것은 0)
```

**Loss Surface 관점**:
- **L2**: 원형 제약 영역 → 부드러운 최소화
- **L1**: 마름모 제약 영역 → 축(axis)에서 만나 희소 해 생성
